...
VERSÃO FINAL - Com quadrados pretos nos cantos e integração Supabase
"""
...
class GeradorProva:
    ...
    def salvar_prova_supabase(self, exam_id, turma, ano, disciplina, questoes, gabarito):
        """Salva os metadados da prova no Supabase"""
        try:
            if not supabase_service.client:
                print("⚠️ Supabase não conectado. Prova não registrada.")
                return False
            ...
            print(f"✅ Prova registrada no Supabase: {exam_id}")
            return True
        except Exception as e:
            print(f"⚠️ Erro ao registrar prova no Supabase: {e}")
            return False
    ...
    def gerar_zip_prova(
        ...
        # =====================================================
        # SALVAR METADADOS DA PROVA NO SUPABASE
        # =====================================================
        try:
            # Extrair gabarito das questões
            gabarito_dict = {}
            for q in questoes[:qtd_questoes]:
                num = q.get("numero", 0)
                alt = q.get("gabarito", "A")
                gabarito_dict[str(num)] = alt

            self.salvar_prova_supabase(
                exam_id=exam_id,
                turma=turma,
                ano=ano,
                disciplina=disciplinas[0] if disciplinas else "Geral",
                questoes=questoes[:qtd_questoes],
                gabarito=gabarito_dict
            )
        except Exception as e:
            print(f"Erro ao salvar metadados da prova: {e}")

...
def show():
    ...
    with tab1:
        ...
        if not usuarios:
            ...
            total = len(usuarios)
            professores = sum(1 for u in usuarios.values() if u.get("perfil") == "professor")
            coordenadores = sum(1 for u in usuarios.values() if u.get("perfil") == "coordenador")
            gestores = sum(1 for u in usuarios.values() if u.get("perfil") == "gestor")
            admins = sum(1 for u in usuarios.values() if u.get("perfil") == "admin")
            ativos = sum(1 for u in usuarios.values() if u.get("ativo", True))
            bloqueados = sum(1 for u in usuarios.values() if not u.get("ativo", True))
